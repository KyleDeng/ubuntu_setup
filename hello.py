        ...
        with self._open_config(filename) as f:
            ...
            for linenr, line in enumerate(f, 1):
                ...
                if match:
                    ...
                    elif sym.orig_type is KEY:
                        match = _conf_key_match(val)
                        if not match:
                            self._warn("malformed key literal in "
                                       "assignment to {}. Assignment ignored."
                                       .format(sym.name_and_loc),
                                       filename, linenr)
                            continue

                        val = unescape(match.group(0))
        ...
        for sym in self.unique_defined_syms:
            if sym.orig_type in _BOOL_TRISTATE:
                ...
                for default, _ in sym.defaults:
                    ...
                    if sym.orig_type is STRING or sym.orig_type is KEY:
...
class Symbol(object):
    ...
    def str_value(self):
        ...
        elif self.orig_type in (STRING, KEY):
...
def expr_value(expr):
    ...
    elif v1.orig_type is KEY and v2.orig_type is KEY:
        # ...then compare them lexicographically
        comp = _strcmp(v1.str_value, v2.str_value)
    else:
...
# A valid right-hand side for an assignment to a key symbol in a .config
# file, including escaped characters. Extracts the contents.
_conf_key_match = _re_match(r'[A-Za-z0-9_]*')
